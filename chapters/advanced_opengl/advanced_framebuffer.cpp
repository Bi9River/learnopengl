...
int scr_width_pixels = 800;
int scr_height_pixels = 600;
...
int main() {
    ...
    glfwGetFramebufferSize(window, &scr_width_pixels, &scr_height_pixels);
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, scr_width_pixels, scr_height_pixels);
    glBindRenderbuffer(GL_RENDERBUFFER,
                       0); // once we've allocated enough memory for the renderbuffer object we can unbind the renderbuffer.
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                              rbo); // attach the renderbuffer object to the depth and stencil attachment of the framebuffer
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cout << "ERROR::FRAMEBUFFER:: Framebuffer is not complete!" << std::endl;
    }
    glBindFramebuffer(GL_FRAMEBUFFER,
                      0); // be sure to unbind the framebuffer to make sure we're not accidentally rendering to the wrong framebuffer.
